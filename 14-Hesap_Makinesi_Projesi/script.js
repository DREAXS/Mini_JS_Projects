...
function sendNumberValue(number) {
  ...
  // Eğer bekleme modundaysak, bu durumda ekrana yeni bir sayı yazdırıyoruz
  if (isWaiting) {
    calculatorTitle.textContent = number;
    isWaiting = false;
  } else {
...
function addDecimal() {
  // Ekranda zaten nokta olup olmadığını kontrol ediyoruz
...
function useOperator(operator) {
  ...
  // Eğer bekleme modundaysak, sadece operatörü güncelliyoruz
  if (operatorValue && isWaiting) {
    operatorValue = operator;
    return;
  }

  // İlk değer atanmadıysa, mevcut değeri başlangıç değeri olarak alıyoruz
  if (!initialValue) {
...
// Hesaplayıcıyı sıfırlayan fonksiyon
function resetAll() {
